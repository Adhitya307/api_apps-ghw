...
class AmbangBatas600H4Model extends Model
{
    protected $DBGroup = 'hdm';
    protected $table = 'ambang_batas_600_h4';
    ...
    /**
     * Update pergerakan from t_pergerakan_elv600 hv_4 * 10
     */
    public function updatePergerakanFromHv4($pengukuran_id)
    {
        $pergerakanModel = new MPergerakanElv600();
        $pergerakan = $pergerakanModel->getByPengukuran($pengukuran_id);
        
        if ($pergerakan && isset($pergerakan['hv_4'])) {
            $pergerakan_value = $pergerakan['hv_4'] * 10;
